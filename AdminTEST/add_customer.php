    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        if (empty($errorMsg)) {
            ...
            if ($emailCount > 0) {
                ...
                $stmt = $conn->prepare("INSERT INTO customers (name, email, phone, address, product_id, package_id, status) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("ssssiis", $name, $email, $phone, $address, $product_id, $package_id, $status);
                ...
                if ($stmt->execute()) {
                    ...
                    $name = $email = $phone = $address = $product_id = $package_id = '';
                } else {
...
<body class="sb-nav-fixed">
                            ...
                            <form method="POST" action="add_customer.php" id="addCustomerForm" novalidate>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="mb-3">
                                            <label for="package_id" class="form-label"><i class="fas fa-cubes"></i> Select Package</label>
                                            <select class="form-select" id="package_id" name="package_id">
                                                <option value="">-- Select Package --</option>
                                                <!-- Packages will be loaded dynamically via JS -->
                                            </select>
                                        </div>
        ...
        fetch(`get_packages.php?product_id=${productId}`)
            ...
            .then(data => {
                data.forEach(pkg => {
                    ...
                    option.value = pkg.id;
                    option.textContent = `${pkg.description} (Rs. ${pkg.amount})`;
    ...
    // Initial load if a product is already selected (e.g., after validation error)
