        ...
        if (empty($errorMsg)) {
            ...
            try {
                ...
                if ($emailCount > 0) {
                    ...
                    $stmt = $conn->prepare("UPDATE customers SET name = ?, email = ?, phone = ?, address = ?, status = ?, product_id = ?, package_id = ? WHERE customer_id = ?");
                    if (!$stmt) {
                        throw new Exception("Database prepare error: " . $conn->error);
                    }

                    $stmt->bind_param("sssssiii", $name, $email, $phone, $address, $status, $product_id, $package_id, $customer_id);
