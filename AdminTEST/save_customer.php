    ...
    if (empty($name)) {
        ...
        if ($email) {
            ...
            if ($result->num_rows > 0) {
                ...
                $updateSql = "UPDATE customers SET name = ?, phone = ?, address = ?, product_id = ?, package_id = ?, status = 'Active' WHERE id = ?";
                $updateStmt = $conn->prepare($updateSql);
                $updateStmt->bind_param("sssiii", $name, $phone, $address, $product_id, $package_id, $customerId);
...
function insertNewCustomer($conn, $name, $email, $phone, $address, $product_id, $package_id, &$response) {
    $insertSql = "INSERT INTO customers (name, email, phone, address, product_id, package_id, status, created_at) VALUES (?, ?, ?, ?, ?, ?, 'Active', NOW())";
    $insertStmt = $conn->prepare($insertSql);
    $insertStmt->bind_param("ssssii", $name, $email, $phone, $address, $product_id, $package_id);
