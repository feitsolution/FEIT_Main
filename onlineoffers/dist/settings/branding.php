...
if ($adminResult->num_rows > 0) {
    ...
    if ($userRoleId !== 1) {
        ...
        header("Location: /onlineoffers/dist/pages/access_denied.php");
        exit();
    }
} else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['logo']) && $_FILES['logo']['error'] == 0) {
        ...
        if (in_array(strtolower($ext), $allowed)) {
            $upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/onlineoffers/dist/uploads/';
            ...
            if (move_uploaded_file($_FILES['logo']['tmp_name'], $destination)) {
                $logo_url = '/onlineoffers/dist/uploads/' . $new_name;
                $logo_uploaded = true;
            } else {
    ...
    if (isset($_FILES['fav_icon']) && $_FILES['fav_icon']['error'] == 0) {
        ...
        if (in_array(strtolower($ext), $allowed)) {
            ...
            if (move_uploaded_file($_FILES['fav_icon']['tmp_name'], $destination)) {
                $fav_icon_url = '/onlineoffers/dist/uploads/' . $new_name;
