...
$countSql = "SELECT COUNT(*) as total 
             ...
             WHERE pb.current_stage = 'Packing' AND pb.status = 'Active' AND pb.quantity > 0";
...
<head>
    <title>Packing Stage - Lily Collection</title>
...
<body>
            ...
            <div class="page-header">
                <div class="page-block">
                    <div class="page-header-title">
                        <h5 class="mb-0 font-medium">Packing Stage</h5>
                    </div>
                </div>
            </div>
            ...
            <div class="main-content-wrapper">
                    ...
                    <form class="tracking-form" method="GET" action="" style="flex-wrap: wrap; gap: 15px;">
                        ...
                        <div class="form-group" style="display: flex; align-items: flex-end;">
                            <div class="button-group">
                                ...
                                <button type="button" class="search-btn" onclick="window.location.href='packing.php'" style="background: #6c757d;">
                ...
                <div class="table-wrapper">
                    <table class="orders-table">
                        ...
                        <tbody>
                            <?php if ($packing_batches && $packing_batches->num_rows > 0): ?>
                                <?php while ($row = $packing_batches->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <button onclick="transitionStage(<?php echo $row['id']; ?>, 'Completed', '<?php echo htmlspecialchars($row['reference_no']); ?>')" class="btn btn-sm btn-success">Complete Batch</button>
                                        </td>
                                    </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr><td colspan="7" class="text-center">No batches in packing stage.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
    ...
    <script>
        function transitionStage(batchId, nextStage, refNo) {
            ...
            const actionText = nextStage === 'Completed' ? 'complete batch ' : 'move batch ';
            if(confirm('Are you sure you want to ' + actionText + refNo + ' with ' + qty + ' units? This will increase product stock.')) {
                $.ajax({
                    url: 'production_actions.php',
                    type: 'POST',
                    data: {action: 'complete_packing', batch_id: batchId, quantity: qty},
                    dataType: 'json',
                    success: function(res) {
                        if(res.success) {
                            alert('Packing completed. Stock updated!');
                            location.reload();
                        } else {
