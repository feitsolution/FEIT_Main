...
if (!$is_main_admin && (!isset($user_data['role_id']) || $user_data['role_id'] != 1)) {
    ...
    header("Location: /OMS_management/dist/pages/access_denied.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_branding'])) {
    ...
    if (isset($_FILES['logo']) && $_FILES['logo']['error'] == 0) {
        ...
        if (in_array(strtolower($ext), $allowed)) {
            $upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/OMS_management/dist/uploads/';
            ...
            if (move_uploaded_file($_FILES['logo']['tmp_name'], $destination)) {
                $logo_url = '/OMS_management/dist/uploads/' . $new_name;
                $logo_uploaded = true;
            } else {
    ...
    if (isset($_FILES['fav_icon']) && $_FILES['fav_icon']['error'] == 0) {
        ...
        if (in_array(strtolower($ext), $allowed)) {
            ...
            if (move_uploaded_file($_FILES['fav_icon']['tmp_name'], $destination)) {
                $fav_icon_url = '/OMS_management/dist/uploads/' . $new_name;
...
<head>
    ...
    <link rel="icon" href="<?php echo !empty($branding['fav_icon_url']) ? $branding['fav_icon_url'] : '/OMS_management/dist/assets/images/favicon.ico'; ?>" type="image/x-icon">
</head>
